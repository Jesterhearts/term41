...
@group(1) @binding(0) var image_tex: texture_2d_array<f32>;
...
fn vs_main(in: VsInput) -> VsOutput {
    ...
    out.uv = in.uv_layer.xy;
    out.layer = i32(in.uv_layer.z);
    return out;
}

@fragment
fn fs_main(in: VsOutput) -> @location(0) vec4<f32> {
    let color = textureSample(image_tex, image_sampler, in.uv, in.layer);
